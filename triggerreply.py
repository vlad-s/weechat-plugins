...
def phelp():
    weechat.prnt("", "Triggerreply (trigge.rs) plugin. Automatically replies \
                over specified triggers")
    weechat.prnt("", "------------")
    weechat.prnt("", "Usage: /triggerreply [list | add trigger:reply | remove \
                trigger | ignore server.#channel | parse server.#channel | \
                ignore user | parse user]")

def create_db():
    tmpcon = sqlite3.connect(dbfile)
    cur = tmpcon.cursor()
    cur.execute("CREATE TABLE \
            triggers(id INTEGER PRIMARY KEY, trig VARCHAR, reply VARCHAR);")
    cur.execute("INSERT INTO \
            triggers(trig, reply) VALUES ('trigge.rs', 'Automatic reply');")
    cur.execute("CREATE TABLE \
            banchans(id INTEGER PRIMARY KEY, ignored VARCHAR);")
    cur.execute("INSERT INTO \
            banchans(ignored) VALUES ('rizon.#help');")
    cur.execute("CREATE TABLE \
            userbans(id INTEGER PRIMARY KEY, nickname VARCHAR);")
    tmpcon.commit()
    cur.close()

def search_trig_cb(data, buffer, date, tags, displayed, highlight, prefix,
                    message):
    """
    ...
    ignored_chan = False
    if not weechat.buffer_get_string(buffer, "name") in cursor.execute(
            "SELECT ignored from banchans;"):
        reply = cursor.execute("SELECT reply FROM triggers \
                WHERE trig=?", str(message))
        weechat.command(buffer, "%s" % reply)
...
def command_input_callback(data, buffer, argv):
    ...
    if len(command) == 0:
        ...
        try:
            ...
            cursor.execute("INSERT INTO triggers(trig, reply) VALUES (?,?)",
                    (trigger, reply,))
        except:
        ...
        try:
            cursor.execute("INSERT INTO banchans(ignored) VALUES (?)",
                    (command[1],))
        except:
        ...
        try:
            cursor.execute("DELETE FROM banchans WHERE ignored = ?",
                    (command[1],))
        except:
